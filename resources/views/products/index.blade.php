    ...
    <div class="relative overflow-hidden bg-gradient-to-br from-violet-700 via-indigo-700 to-violet-800 text-white">
        ...
        <div class="relative max-w-[1850px] mx-auto px-4 sm:px-6 lg:px-8 py-10">
            <nav class="text-xs font-bold text-violet-100/80 mb-3 flex items-center gap-2">
                <a href="{{ route('home') }}" class="hover:text-white"><i class="fa-solid fa-house"></i> {{ __('products.breadcrumb_home') }}</a>
                <i class="fa-solid fa-chevron-{{ $isRtl ? 'left' : 'right' }} text-[8px]"></i>
                <span class="text-white">{{ __('products.breadcrumb_products') }}</span>
            </nav>
            <div class="flex flex-wrap items-end justify-between gap-4">
                <div>
                    <h1 class="text-3xl sm:text-4xl font-black tracking-tight">{{ $pageTitle }}</h1>
                    <p class="text-violet-100 mt-2 max-w-2xl">{{ $pageSubtitle ?: __('products.total_products', ['count' => $products->total()]) }}</p>
                </div>
                <div class="flex items-center gap-2 text-xs font-bold">
                    <span class="inline-flex items-center gap-1.5 bg-white/15 backdrop-blur border border-white/20 px-3 py-1.5 rounded-full">
                        <i class="fa-solid fa-box-open"></i> {{ __('products.items_count', ['count' => $products->total()]) }}
                    </span>
                    @if($activeCollege)
    ...
    <div class="max-w-[1850px] mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <form action="{{ route('products.index') }}" method="get" class="flex flex-wrap gap-3 items-center mb-8">
            <div class="relative flex-1 min-w-[220px]">
                ...
                <input type="search" name="search" value="{{ request('search') }}" placeholder="{{ __('products.search_placeholder') }}"
            ...
            <select name="sort" onchange="this.form.submit()" aria-label="{{ __('products.sort_label') }}"
                ...
                <option value="newest" @selected(request('sort', 'newest') === 'newest')>{{ __('products.sort_newest') }}</option>
                <option value="price_asc" @selected(request('sort') === 'price_asc')>{{ __('products.sort_price_asc') }}</option>
                <option value="price_desc" @selected(request('sort') === 'price_desc')>{{ __('products.sort_price_desc') }}</option>
                <option value="name" @selected(request('sort') === 'name')>{{ __('products.sort_name') }}</option>
            </select>
            <button type="button" id="mobile-filter-btn"
                ...
                <i class="fa-solid fa-sliders text-violet-600"></i> {{ __('products.mobile_filters') }}
            </button>
                    ...
                    <div class="bg-white border border-slate-200 rounded-3xl shadow-sm overflow-hidden">
                        ...
                        <div class="flex items-center justify-between px-5 py-4 bg-gradient-to-r from-violet-50 to-indigo-50 border-b border-slate-100">
                            <div class="flex items-center gap-2.5">
                                ...
                                <h3 class="text-sm font-black text-slate-900">{{ __('products.filters_title') }}</h3>
                            </div>
                            @if(request()->hasAny(['college','department','search','featured','in_stock','on_sale','min_price','max_price']))
                                <a href="{{ route('products.index') }}" class="text-[11px] font-bold text-rose-600 hover:text-rose-700 inline-flex items-center gap-1">
                                    <i class="fa-solid fa-rotate-left"></i> {{ __('products.filters_reset') }}
                                </a>
                            @endif
                        </div>
                        ...
                        <form id="filter-form" action="{{ route('products.index') }}" method="get" class="p-5 space-y-6">
                            ...
                            {{-- College --}}
                            <div>
                                <label class="block text-[10px] font-extrabold text-slate-400 uppercase tracking-widest mb-3">{{ __('products.filter_college') }}</label>
                                <div class="space-y-1 max-h-64 overflow-y-auto pr-1 -mr-1">
                                    <a href="{{ route('products.index', request()->except(['college','department','page'])) }}"
                                       class="flex items-center justify-between px-3 py-2.5 rounded-xl text-xs font-bold transition-all {{ !$activeCollege ? 'bg-violet-50 border border-violet-200 text-violet-700' : 'text-slate-600 hover:bg-slate-50 border border-transparent' }}">
                                        <span class="flex items-center gap-2"><i class="fa-solid fa-layer-group text-[10px] opacity-60"></i> {{ __('products.filter_all_colleges') }}</span>
                            ...
                            @if($activeCollege && $activeCollege->children->isNotEmpty())
                                <div>
                                    <label class="block text-[10px] font-extrabold text-slate-400 uppercase tracking-widest mb-3">{{ __('products.filter_department') }}</label>
                            ...
                            <div class="pt-2 border-t border-slate-100">
                                <label class="block text-[10px] font-extrabold text-slate-400 uppercase tracking-widest mb-3">{{ __('products.filter_price') }}</label>
                                <div class="flex items-center gap-2">
                                    <div class="relative flex-1">
                                        <input type="number" name="min_price" value="{{ request('min_price') }}" min="0" placeholder="{{ __('products.filter_min') }}"
                                    ...
                                    <div class="relative flex-1">
                                        <input type="number" name="max_price" value="{{ request('max_price') }}" min="0" placeholder="{{ __('products.filter_max') }}"
                                               class="w-full h-10 px-3 bg-slate-50 border border-slate-200 rounded-xl text-xs font-semibold outline-none focus:border-violet-400 focus:bg-white">
                                    </div>
                                </div>
                                <button type="submit" class="mt-3 w-full h-9 bg-slate-900 hover:bg-violet-600 text-white text-[11px] font-black uppercase tracking-wider rounded-xl transition">
                                    {{ __('products.filter_apply') }}
                                </button>
                            </div>

                            {{-- Options toggles --}}
                            <div class="pt-4 border-t border-slate-100">
                                <label class="block text-[10px] font-extrabold text-slate-400 uppercase tracking-widest mb-3">{{ __('products.filter_options') }}</label>
                                <div class="space-y-2">
                                    <label class="flex items-center justify-between gap-2 px-3 py-2.5 rounded-xl border border-slate-200 hover:border-violet-300 cursor-pointer transition group">
                                        <span class="flex items-center gap-2 text-xs font-bold text-slate-700">
                                            <i class="fa-solid fa-star text-amber-500 group-hover:scale-110 transition"></i> {{ __('products.filter_featured') }}
                                        </span>
                                    ...
                                    <label class="flex items-center justify-between gap-2 px-3 py-2.5 rounded-xl border border-slate-200 hover:border-violet-300 cursor-pointer transition group">
                                        <span class="flex items-center gap-2 text-xs font-bold text-slate-700">
                                            <i class="fa-solid fa-circle-check text-emerald-500 group-hover:scale-110 transition"></i> {{ __('products.filter_in_stock') }}
                                        </span>
                                    ...
                                    <label class="flex items-center justify-between gap-2 px-3 py-2.5 rounded-xl border border-rose-200/70 bg-rose-50/40 hover:border-rose-300 cursor-pointer transition group">
                                        <span class="flex items-center gap-2 text-xs font-bold text-rose-700">
                                            <i class="fa-solid fa-tags text-rose-500 group-hover:scale-110 transition"></i> {{ __('products.filter_on_sale') }}
                                        </span>
            ...
            <div id="filter-drawer" class="fixed inset-0 z-50 hidden lg:hidden">
                ...
                <div class="absolute {{ $isRtl ? 'left-0' : 'right-0' }} top-0 h-full w-[min(340px,92vw)] bg-white shadow-2xl p-6 overflow-y-auto">
                    <div class="flex items-center justify-between mb-6">
                        <h3 class="font-black text-slate-900">{{ __('products.filters_title') }}</h3>
            ...
            <div id="products-results" class="flex-1 min-w-0 transition-opacity">
                ...
                @if($activeCollege || $activeDepartment || request('search') || request('on_sale') || request('featured') || request('in_stock'))
                    <div class="flex flex-wrap items-center gap-2 mb-6">
                        <span class="text-[10px] font-extrabold text-slate-400 uppercase tracking-widest">{{ __('products.active_filters') }}:</span>
                        @if(request('search'))
                            <span class="inline-flex items-center gap-1.5 px-3 py-1 bg-violet-100 text-violet-700 rounded-full text-xs font-bold">
                                {{ __('products.search_chip') }}: {{ request('search') }}
                        ...
                        @endif
                        @if(request('featured'))
                            <span class="inline-flex items-center gap-1.5 px-3 py-1 bg-amber-100 text-amber-700 rounded-full text-xs font-bold"><i class="fa-solid fa-star"></i> {{ __('products.filter_featured') }}
                                <a href="{{ route('products.index', request()->except(['featured','page'])) }}"><i class="fa-solid fa-xmark"></i></a>
                            </span>
                        @endif
                        @if(request('in_stock'))
                            <span class="inline-flex items-center gap-1.5 px-3 py-1 bg-emerald-100 text-emerald-700 rounded-full text-xs font-bold"><i class="fa-solid fa-circle-check"></i> {{ __('products.filter_in_stock') }}
                                <a href="{{ route('products.index', request()->except(['in_stock','page'])) }}"><i class="fa-solid fa-xmark"></i></a>
                            </span>
                        @endif
                        @if(request('on_sale'))
                            <span class="inline-flex items-center gap-1.5 px-3 py-1 bg-rose-100 text-rose-700 rounded-full text-xs font-bold"><i class="fa-solid fa-tags"></i> {{ __('products.filter_on_sale') }}
                ...
                @else
                    <div class="text-center py-20 bg-white rounded-3xl border border-slate-200">
                        ...
                        <h3 class="text-lg font-bold text-slate-800">{{ __('products.empty_title') }}</h3>
                        <p class="text-slate-500 text-sm mt-2">{{ __('products.empty_subtitle') }}</p>
                        <a href="{{ route('products.index') }}" class="inline-block mt-4 text-violet-600 font-bold text-sm">{{ __('products.empty_clear') }}</a>
                    </div>
                @endif
            </div>
        </div>
