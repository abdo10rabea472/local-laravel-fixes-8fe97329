...
<div class="space-y-6">
    <div class="flex items-center justify-between">
        ...
        <a href="{{ route('account.returns.index') }}" class="text-violet-600 text-sm font-semibold">{{ __('app.acc_my_returns_link') }}</a>
    </div>
    ...
    <div class="bg-white rounded-2xl border p-6">
        <div class="flex flex-wrap items-center justify-between gap-3 mb-6">
            ...
            <p class="text-sm text-slate-500">{{ __('app.acc_order') }}: <strong>{{ $return->order?->order_number }}</strong></p>
        </div>
        ...
        <table class="w-full text-sm">
            <thead class="text-xs text-slate-500 uppercase border-b">
                <tr>
                    ...
                    <th class="py-2 text-right">{{ __('app.acc_quantity') }}</th>
                    <th class="py-2 text-right">{{ __('app.acc_price') }}</th>
                    <th class="py-2 text-right">{{ __('app.acc_total') }}</th>
                </tr>
            </thead>
        ...
        <div class="border-t mt-4 pt-4 grid sm:grid-cols-2 gap-4 text-sm">
            <div><strong class="text-slate-500">{{ __('app.acc_reason_label') }}</strong> {{ $return->reasonLabel() }}</div>
            <div><strong class="text-slate-500">{{ __('app.acc_expected_amount') }}</strong> {{ number_format($return->refund_amount, 2) }} {{ __('app.cat_egp') }}</div>
        </div>

        @if($return->customer_note)
            <div class="mt-4 p-4 bg-slate-50 rounded-xl text-sm">
                <strong class="text-slate-500 block mb-1">{{ __('app.acc_your_note') }}</strong>{{ $return->customer_note }}
            </div>
        @endif

        @if($return->admin_note)
            <div class="mt-4 p-4 bg-violet-50 rounded-xl text-sm">
                <strong class="text-violet-700 block mb-1">{{ __('app.acc_admin_note') }}</strong>{{ $return->admin_note }}
            </div>
        @endif
    </div>
</div>
