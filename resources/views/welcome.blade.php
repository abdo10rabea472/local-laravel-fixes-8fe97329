...
{{-- ═══════════════ CTA BANNER — FUTURISTIC BULK ORDERS ═══════════════ --}}
@php
    $ctaBadge    = site_setting('cta_badge',    __('home.cta_badge'));
    $ctaTitle    = site_setting('cta_title',    __('home.cta_title'));
    $ctaSubtitle = site_setting('cta_subtitle', __('home.cta_subtitle'));
    $ctaButton   = site_setting('cta_button',   __('home.cta_button'));
    $ctaUrl      = site_setting('cta_url') ?: route('products.index');
    $ctaBg       = site_setting_url('cta_bg_image');
    $ctaImages   = collect([1,2,3,4])
        ->map(fn($i) => site_setting_url('cta_image_'.$i))
        ->filter()->values();
@endphp
<section class="py-12 bg-slate-50">
    <div class="max-w-[1850px] mx-auto px-4 sm:px-6 lg:px-8">
        <div class="relative rounded-[2rem] overflow-hidden text-white shadow-2xl ring-1 ring-white/10
                    bg-gradient-to-br from-[#0b1026] via-[#1a1148] to-[#3d1466]">

            {{-- Background image with dark overlay --}}
            @if($ctaBg)
                <div class="absolute inset-0">
                    <img src="{{ $ctaBg }}" alt="" class="w-full h-full object-cover opacity-30">
                    <div class="absolute inset-0 bg-gradient-to-br from-[#0b1026]/90 via-[#1a1148]/80 to-[#3d1466]/85"></div>
                </div>
            @endif

            {{-- Futuristic grid + glow accents --}}
            <div class="absolute inset-0 opacity-[0.18] pointer-events-none"
                 style="background-image:linear-gradient(rgba(255,255,255,.25) 1px,transparent 1px),linear-gradient(90deg,rgba(255,255,255,.25) 1px,transparent 1px);background-size:42px 42px;mask-image:radial-gradient(ellipse at center,#000 30%,transparent 75%)"></div>
            <div class="absolute -right-24 -top-24 w-96 h-96 bg-fuchsia-500/30 rounded-full blur-3xl animate-pulse"></div>
            <div class="absolute -left-24 -bottom-24 w-[28rem] h-[28rem] bg-cyan-400/20 rounded-full blur-3xl"></div>
            <div class="absolute top-1/2 left-1/3 w-40 h-40 bg-amber-300/20 rounded-full blur-2xl"></div>

            {{-- Animated corner brackets --}}
            <span class="absolute top-5 left-5 w-8 h-8 border-t-2 border-l-2 border-cyan-300/70 rounded-tl-2xl"></span>
            <span class="absolute top-5 right-5 w-8 h-8 border-t-2 border-r-2 border-cyan-300/70 rounded-tr-2xl"></span>
            <span class="absolute bottom-5 left-5 w-8 h-8 border-b-2 border-l-2 border-cyan-300/70 rounded-bl-2xl"></span>
            <span class="absolute bottom-5 right-5 w-8 h-8 border-b-2 border-r-2 border-cyan-300/70 rounded-br-2xl"></span>

            <div class="relative grid grid-cols-1 lg:grid-cols-2 gap-10 items-center p-8 sm:p-12 lg:p-16">

                {{-- Copy --}}
                <div>
                    <span class="inline-flex items-center gap-2 bg-white/10 backdrop-blur border border-cyan-300/40 text-cyan-200 text-[11px] font-black uppercase tracking-[0.2em] px-4 py-2 rounded-full mb-5">
                        <span class="w-1.5 h-1.5 rounded-full bg-cyan-300 animate-pulse"></span>
                        {{ $ctaBadge }}
                    </span>
                    <h3 class="text-3xl sm:text-4xl lg:text-5xl font-black mb-4 leading-tight
                               bg-gradient-to-r from-white via-amber-100 to-cyan-200 bg-clip-text text-transparent drop-shadow">
                        {{ $ctaTitle }}
                    </h3>
                    <p class="text-violet-100/90 text-base sm:text-lg mb-7 max-w-lg leading-relaxed">
                        {{ $ctaSubtitle }}
                    </p>

                    <div class="flex flex-wrap items-center gap-4">
                        <a href="{{ $ctaUrl }}"
                           class="group relative inline-flex items-center gap-3 px-7 py-3.5 rounded-2xl font-black text-sm
                                  bg-gradient-to-r from-amber-300 via-amber-400 to-amber-300 text-violet-950
                                  shadow-[0_10px_40px_-10px_rgba(252,211,77,0.6)]
                                  hover:shadow-[0_15px_50px_-10px_rgba(252,211,77,0.9)]
                                  hover:-translate-y-0.5 transition-all duration-300">
                            <span>{{ $ctaButton }}</span>
                            <i class="fa-solid fa-arrow-right transition-transform group-hover:translate-x-1"></i>
                            <span class="absolute inset-0 rounded-2xl ring-2 ring-amber-200/0 group-hover:ring-amber-200/60 transition"></span>
                        </a>
                        <div class="flex items-center gap-2 text-xs text-cyan-100/80">
                            <i class="fa-solid fa-shield-halved text-cyan-300"></i>
                            <span class="font-bold">Certified · ISO grade</span>
                        </div>
                    </div>
                </div>

                {{-- Visual: image gallery / icon grid --}}
                <div class="relative hidden lg:block">
                    @if($ctaImages->count() > 0)
                        <div class="grid grid-cols-2 gap-4">
                            @foreach($ctaImages as $idx => $img)
                                <div class="relative aspect-square rounded-2xl overflow-hidden group ring-1 ring-white/15
                                            {{ $idx % 2 === 0 ? 'translate-y-0' : 'translate-y-6' }}">
                                    <img src="{{ $img }}" alt="" class="w-full h-full object-cover transition duration-700 group-hover:scale-110">
                                    <div class="absolute inset-0 bg-gradient-to-t from-violet-950/80 via-transparent to-transparent"></div>
                                    <div class="absolute inset-0 ring-1 ring-cyan-300/0 group-hover:ring-cyan-300/60 rounded-2xl transition"></div>
                                </div>
                            @endforeach
                        </div>
                    @else
                        <div class="grid grid-cols-3 gap-3">
                            @foreach(['fa-microscope','fa-flask','fa-dna','fa-stethoscope','fa-calculator','fa-screwdriver-wrench'] as $i => $ic)
                                <div class="aspect-square rounded-2xl bg-white/5 backdrop-blur border border-white/15
                                            flex items-center justify-center text-3xl text-cyan-100/90
                                            hover:bg-white/15 hover:border-cyan-300/60 hover:-translate-y-1
                                            transition duration-300 relative overflow-hidden group
                                            {{ $i % 2 === 0 ? '' : 'translate-y-4' }}">
                                    <span class="absolute inset-0 bg-gradient-to-br from-fuchsia-400/0 to-cyan-400/0 group-hover:from-fuchsia-400/20 group-hover:to-cyan-400/20 transition"></span>
                                    <i class="fa-solid {{ $ic }} relative"></i>
                                </div>
                            @endforeach
                        </div>
                    @endif
                    ...
                    {{-- Floating stat chip --}}
                    <div class="absolute -bottom-4 -left-4 bg-white/10 backdrop-blur-xl border border-white/20 rounded-2xl px-4 py-3 shadow-xl">
                        <div class="flex items-center gap-3">
                            ...
                            <div>
                                <div class="text-[10px] uppercase tracking-widest text-cyan-200/80 font-bold">Bulk shipping</div>
                                <div class="text-sm font-black text-white">Free over 50 units</div>
                            </div>
                        </div>
                    </div>
                </div>
