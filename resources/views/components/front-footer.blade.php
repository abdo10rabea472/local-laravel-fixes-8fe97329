...
<section class="bg-gradient-to-r from-violet-700 via-indigo-700 to-violet-800 text-white">
    <div class="max-w-[1850px] mx-auto px-4 sm:px-6 lg:px-8 py-6 grid grid-cols-2 lg:grid-cols-4 gap-5">
        @php
            $footerFeatures = [
                ['fa-truck-fast', __('footer.feat_delivery_t'), __('footer.feat_delivery_s')],
                ['fa-shield-halved', __('footer.feat_cert_t'), __('footer.feat_cert_s')],
                ['fa-rotate-left', __('footer.feat_returns_t'), __('footer.feat_returns_s')],
                ['fa-headset', __('footer.feat_support_t'), __('footer.feat_support_s')],
            ];
        @endphp
...
<footer class="relative bg-slate-950 text-slate-400 pt-16 pb-6 overflow-hidden">
    ...
    <div class="relative max-w-[1850px] mx-auto px-4 sm:px-6 lg:px-8 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-12 gap-10">
        ...
        <div class="lg:col-span-3">
            <h5 class="text-white font-black text-sm mb-4 uppercase tracking-wider"><span class="inline-block w-1 h-4 bg-violet-500 rounded-full align-middle mr-2"></span>{{ __('footer.colleges') }}</h5>
            <ul class="space-y-2 text-sm">
                ...
                <li><a href="{{ route('products.index') }}" class="hover:text-violet-400 transition">{{ __('footer.all_categories') }}</a></li>
                @endif
            </ul>
        </div>
        ...
        <div class="lg:col-span-2">
            <h5 class="text-white font-black text-sm mb-4 uppercase tracking-wider"><span class="inline-block w-1 h-4 bg-violet-500 rounded-full align-middle mr-2"></span>{{ __('footer.shop') }}</h5>
            <ul class="space-y-2 text-sm">
                @if(($navFooterMenu ?? collect())->isNotEmpty())
                    ...
                    @endforeach
                @else
                    <li><a href="{{ route('products.index') }}" class="hover:text-violet-400 transition">{{ __('footer.all_products') }}</a></li>
                    <li><a href="{{ route('products.index', ['featured' => 1]) }}" class="hover:text-violet-400 transition">{{ __('footer.featured') }}</a></li>
                    <li><a href="{{ route('products.index', ['sort' => 'newest']) }}" class="hover:text-violet-400 transition">{{ __('footer.new_arrivals') }}</a></li>
                    <li><a href="{{ url('/') }}#colleges" class="hover:text-violet-400 transition">{{ __('footer.by_college') }}</a></li>
                @endif
            </ul>
        </div>
        ...
        {{-- Account / Help --}}
        <div class="lg:col-span-3">
            <h5 class="text-white font-black text-sm mb-4 uppercase tracking-wider"><span class="inline-block w-1 h-4 bg-violet-500 rounded-full align-middle mr-2"></span>{{ __('footer.help_account') }}</h5>
            <ul class="space-y-2 text-sm">
                <li><a href="{{ route('about') }}" class="hover:text-violet-400 transition">{{ __('footer.about_us') }}</a></li>
                <li><a href="{{ route('contact') }}" class="hover:text-violet-400 transition">{{ __('footer.contact_us') }}</a></li>
                <li><a href="{{ route('blog.index') }}" class="hover:text-violet-400 transition">{{ __('footer.blog') }}</a></li>
                <li><a href="{{ route('offers') }}" class="hover:text-violet-400 transition">{{ __('footer.offers') }}</a></li>
                <li><a href="{{ route('track-order') }}" class="hover:text-violet-400 transition">{{ __('footer.track_order') }}</a></li>
                <li><a href="{{ route('compare.index') }}" class="hover:text-violet-400 transition">{{ __('footer.compare') }}</a></li>
                <li><a href="{{ route('pages.faqs') }}" class="hover:text-violet-400 transition">{{ __('footer.faqs') }}</a></li>
                <li><a href="{{ route('pages.returns') }}" class="hover:text-violet-400 transition">{{ __('footer.returns_refunds') }}</a></li>
                <li><a href="{{ route('pages.privacy') }}" class="hover:text-violet-400 transition">{{ __('footer.privacy') }}</a></li>
                @auth('web')
                    <li><a href="{{ route('account.dashboard') }}" class="hover:text-violet-400 transition">{{ __('footer.my_dashboard') }}</a></li>
                    <li><a href="{{ route('wishlist.index') }}" class="hover:text-violet-400 transition">{{ __('footer.my_wishlist') }}</a></li>
                @else
                    <li><a href="{{ route('login') }}" class="hover:text-violet-400 transition">{{ __('footer.sign_in') }}</a></li>
                    <li><a href="{{ route('register') }}" class="hover:text-violet-400 transition">{{ __('footer.create_account') }}</a></li>
                @endauth
            </ul>

            {{-- Mini newsletter --}}
            <div class="mt-6">
                <p class="text-xs font-bold text-white mb-2">{{ __('footer.mini_news') }}</p>
                ...
                <form action="{{ route('newsletter.subscribe') }}" method="post" class="flex gap-2">
                    @csrf
                    <input type="email" name="email" required placeholder="{{ __('footer.mini_email') }}" class="flex-1 h-10 px-3 bg-slate-900 border border-slate-800 rounded-lg text-xs text-white placeholder:text-slate-600 outline-none focus:border-violet-500">
    ...
    <div class="relative max-w-[1850px] mx-auto px-4 sm:px-6 lg:px-8 mt-12 pt-6 border-t border-slate-800/80">
        <div class="flex flex-col md:flex-row items-center justify-between gap-4 text-xs">
            <p class="text-slate-500">
                © {{ date('Y') }} <span class="text-slate-300 font-bold">UNI-LAB MARKET</span>. {{ __('footer.copyright') }}
            </p>
            <div class="flex items-center gap-2 flex-wrap justify-center">
                <span class="text-slate-600 font-bold uppercase tracking-wider text-[10px]">{{ __('footer.we_accept') }}</span>
