...
<div class="p-6 space-y-6">
    <h1 class="text-2xl font-bold text-slate-800 dark:text-gray-100">{{ __('app.admin_reports_coupons_heading') }}</h1>

    <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div class="bg-white border rounded-2xl p-5 dark:bg-dark-900 dark:border-dark-700">
            <p class="text-xs font-bold text-slate-500 dark:text-gray-400">{{ __('app.admin_reports_coupons_kpi_redemptions') }}</p>
            <h3 class="text-2xl font-black text-violet-600 mt-2">{{ (int) $totals->redemptions }}</h3>
        </div>
        <div class="bg-white border rounded-2xl p-5 dark:bg-dark-900 dark:border-dark-700">
            <p class="text-xs font-bold text-slate-500 dark:text-gray-400">{{ __('app.admin_reports_coupons_kpi_discounts') }}</p>
            <h3 class="text-2xl font-black text-rose-600 mt-2">{{ money($totals->discount) }}</h3>
        </div>
        <div class="bg-white border rounded-2xl p-5 dark:bg-dark-900 dark:border-dark-700">
            <p class="text-xs font-bold text-slate-500 dark:text-gray-400">{{ __('app.admin_reports_coupons_kpi_revenue') }}</p>
    ...
    <div class="bg-white border rounded-2xl overflow-hidden dark:bg-dark-900 dark:border-dark-700">
        <div class="px-5 py-4 border-b">
            <h3 class="font-bold text-slate-800 dark:text-gray-100">{{ __('app.admin_reports_coupons_section_all') }}</h3>
        </div>
        <table class="w-full text-sm">
            ...
            <tbody class="divide-y">
                @forelse($coupons as $c)
                    <tr>
                        <td class="px-4 py-3 font-mono font-bold text-slate-800 dark:text-gray-100">{{ $c->code }}</td>
                        <td class="px-4 py-3">
                            @if($c->type === 'percent')
                ...
                @empty
                    <tr><td colspan="6" class="text-center py-12 text-slate-400 dark:text-gray-500">{{ __('app.admin_reports_coupons_no_coupons') }}</td></tr>
                @endforelse
            </tbody>
        </table>
