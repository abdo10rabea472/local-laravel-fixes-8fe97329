...
@section('title', __('app.admin_colleges_title'))

@section('content')
<x-admin.page :title="__('app.admin_colleges_heading')" :subtitle="__('app.admin_colleges_subtitle')">
    <x-admin.card :title="__('app.admin_colleges_card_title')" icon="fa-building-columns" padding="p-0">
        <div class="overflow-x-auto">
            <table class="w-full text-sm">
                <thead class="bg-gray-50 dark:bg-dark-800 text-gray-500 dark:text-gray-400 text-xs">
                    <tr>
                        <th class="text-left p-4">{{ __('app.admin_colleges_col_college') }}</th>
                        <th class="text-left p-4">{{ __('app.admin_colleges_col_colors') }}</th>
                        <th class="text-left p-4">{{ __('app.admin_colleges_col_subcats') }}</th>
                        <th class="text-left p-4">{{ __('app.admin_colleges_col_products') }}</th>
                        <th class="text-left p-4">{{ __('app.admin_common_status') }}</th>
                    ...
                    <tr class="border-t border-gray-100 dark:border-gray-800 hover:bg-gray-50 dark:hover:bg-dark-800/50">
                        ...
                        <td class="p-4">
                            <span class="px-2 py-1 rounded-full text-xs font-bold {{ $college->status ? 'bg-emerald-50 dark:bg-emerald-950/30 text-emerald-700 dark:text-emerald-400' : 'bg-gray-100 dark:bg-dark-800 text-gray-500' }}">
                                {{ $college->status ? __('app.admin_common_active') : __('app.admin_common_disabled') }}
                            </span>
                        </td>
                        <td class="p-4">
                            <div class="flex gap-3">
                                <a href="{{ route('admin.colleges.edit', $college) }}" class="text-primary-600 hover:text-primary-700" title="{{ __('app.admin_common_edit') }}"><i class="fa-solid fa-pen"></i></a>
                                <a href="{{ route('admin.subcategories.index', ['college_id' => $college->id]) }}" class="text-indigo-500 hover:text-indigo-600" title="{{ __('app.admin_menu_products_subcats') }}"><i class="fa-solid fa-sitemap"></i></a>
                                <a href="{{ route('category.show', $college->slug) }}" target="_blank" class="text-gray-500 hover:text-gray-700" title="{{ __('app.admin_common_preview') }}"><i class="fa-solid fa-eye"></i></a>
                                <form method="POST" action="{{ route('admin.colleges.destroy', $college) }}" data-ajax-confirm="{{ __('app.admin_colleges_confirm_delete') }}" data-ajax-remove>
                                    @csrf @method('DELETE')
    ...
    <x-slot:side>
        <x-admin.card :title="__('app.admin_common_quick_actions')" icon="fa-bolt">
            <a href="{{ route('admin.colleges.create') }}" class="w-full h-12 inline-flex items-center justify-center gap-2 bg-primary-600 hover:bg-primary-700 text-white font-bold rounded-xl shadow-lg shadow-primary-500/20 transition-colors">
                <i class="fa-solid fa-plus"></i> {{ __('app.admin_colleges_add') }}
            </a>
            <a href="{{ route('admin.subcategories.index') }}" class="mt-2 w-full h-11 inline-flex items-center justify-center gap-2 bg-gray-100 dark:bg-dark-800 text-gray-700 dark:text-gray-300 font-bold rounded-xl hover:bg-gray-200 dark:hover:bg-dark-700 transition-colors">
                <i class="fa-solid fa-sitemap"></i> {{ __('app.admin_menu_products_subcats') }}
            </a>
        </x-admin.card>
    </x-slot:side>
</x-admin.page>
