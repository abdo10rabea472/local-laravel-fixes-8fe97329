...
<form method="POST" action="{{ $category->exists ? route('admin.colleges.update', $category) : route('admin.colleges.store') }}" enctype="multipart/form-data">
    ...
    <x-admin.page :title="$title" :subtitle="__('app.admin_college_form_subtitle')" :back="route('admin.colleges.index')" :backLabel="__('app.admin_college_form_back')">
        <x-admin.card :title="__('app.admin_college_form_info')" icon="fa-building-columns">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    <label class="text-xs font-bold text-gray-500 dark:text-gray-400 block mb-1.5">{{ __('app.admin_college_form_name') }} *</label>
                    <input type="text" name="name" value="{{ old('name', $category->name) }}" required placeholder="{{ __('app.admin_college_form_name_ph') }}"
                ...
                <div>
                    ...
                    <input type="text" name="slug" value="{{ old('slug', $category->slug) }}" placeholder="{{ __('app.admin_college_form_slug_ph') }}"
            ...
            <div class="mt-4">
                ...
                <textarea name="description" rows="4" placeholder="{{ __('app.admin_college_form_description_ph') }}"
        ...
        <x-admin.card :title="__('app.admin_college_form_icon_banner')" icon="fa-image">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    <label class="text-xs font-bold text-gray-500 dark:text-gray-400 block mb-1.5">{{ __('app.admin_college_form_icon') }}</label>
                    @if($category->image)
                ...
                <div>
                    <label class="text-xs font-bold text-gray-500 dark:text-gray-400 block mb-1.5">{{ __('app.admin_college_form_banner') }}</label>
                    @if($category->banner)
                    <div class="mb-3">
        ...
        <x-admin.card :title="__('app.admin_college_form_colors')" icon="fa-palette">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    <label class="text-xs font-bold text-gray-500 dark:text-gray-400 block mb-1.5">{{ __('app.admin_college_form_primary_color') }}</label>
                ...
                <div>
                    <label class="text-xs font-bold text-gray-500 dark:text-gray-400 block mb-1.5">{{ __('app.admin_college_form_secondary_color') }}</label>
                    <div class="flex items-center gap-3">
            ...
            <div class="mt-4 p-4 rounded-xl"
                ...
                <p class="text-white font-bold text-sm">{{ __('app.admin_college_form_color_preview') }}</p>
            </div>
        </x-admin.card>
        ...
        <x-admin.card :title="__('app.admin_common_seo_settings')" icon="fa-magnifying-glass-chart">
            <div class="space-y-3">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                    <input type="text" name="og_title" value="{{ old('og_title', $category->og_title) }}" placeholder="{{ __('app.admin_common_og_title') }}"
                           class="w-full h-11 px-4 bg-gray-50 dark:bg-dark-800 border border-gray-200 dark:border-gray-700 rounded-xl text-sm focus:border-primary-500 focus:outline-none">
                    <input type="text" name="og_description" value="{{ old('og_description', $category->og_description) }}" placeholder="{{ __('app.admin_common_og_description') }}"
                           class="w-full h-11 px-4 bg-gray-50 dark:bg-dark-800 border border-gray-200 dark:border-gray-700 rounded-xl text-sm focus:border-primary-500 focus:outline-none">
                </div>
                <textarea name="schema_markup" rows="3" placeholder="{{ __('app.admin_common_schema') }}"
        ...
        <x-slot:side>
            <x-admin.card :title="__('app.admin_college_form_status_card')" icon="fa-toggle-on">
                <label class="flex items-center gap-2 text-sm font-semibold text-gray-700 dark:text-gray-200 cursor-pointer">
                    ...
                    {{ __('app.admin_college_form_active') }}
                </label>
                ...
                <div class="mt-5 pt-5 border-t border-gray-100 dark:border-gray-800 space-y-2">
                    <button type="submit" class="w-full h-12 bg-primary-600 hover:bg-primary-700 text-white font-bold rounded-xl shadow-lg shadow-primary-500/20 transition-colors">
                        ...
                        {{ $category->exists ? __('app.admin_common_save_changes') : __('app.admin_colleges_add') }}
                    </button>
