...
<div class="p-6 space-y-6" x-data="{ showForm: false, editing: null }">
    <div class="flex items-center justify-between">
        <h1 class="text-2xl font-bold text-slate-800 dark:text-white">{{ __('app.admin_carriers_heading') }}</h1>
        ...
        <div class="flex items-center gap-3">
            ...
            <div>
                ...
                <p class="text-xs text-slate-600">
                    @if(!$aramexInstalled)
                        <span class="text-rose-600 font-semibold">{{ __('app.admin_carriers_aramex_not_installed') }}</span> {{ __('app.admin_carriers_aramex_run') }}: <code class="bg-white px-2 py-0.5 rounded">composer require octw/aramex</code>
                    @elseif(!$aramexConfigured)
                        <span class="text-amber-700 font-semibold">{{ __('app.admin_carriers_aramex_not_configured') }}</span> {{ __('app.admin_carriers_aramex_publish') }}: <code class="bg-white px-2 py-0.5 rounded">php artisan vendor:publish --provider="Octw\Aramex\AramexServiceProvider"</code>
                    @else
                        <span class="text-emerald-700 font-semibold">✓ {{ __('app.admin_carriers_aramex_ready') }}</span>
                    @endif
                </p>
            </div>
        </div>
    ...
    <div x-show="showForm" x-transition class="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4" @click.self="showForm = false">
        <div class="bg-white rounded-2xl shadow-2xl max-w-lg w-full p-6 max-h-[90vh] overflow-y-auto">
            <h2 class="text-lg font-bold text-slate-800 mb-4" x-text="editing ? '{{ __('app.admin_carriers_edit') }}' : '{{ __('app.admin_carriers_add') }}'"></h2>
            <form :action="editing ? `/admin/shipping-carriers/${editing.id}` : '{{ route('admin.shipping-carriers.store') }}'" method="POST" class="space-y-4">
                ...
                <div>
                    <label class="block text-xs font-semibold text-slate-700 mb-1">{{ __('app.admin_carriers_name') }} *</label>
                    <input type="text" name="name" :value="editing?.name" required class="w-full rounded-xl border-slate-200">
                </div>
                <div>
                    <label class="block text-xs font-semibold text-slate-700 mb-1">{{ __('app.admin_carriers_code') }} *</label>
                ...
                <div>
                    <label class="block text-xs font-semibold text-slate-700 mb-1">{{ __('app.admin_carriers_tracking_url') }}</label>
                ...
                <div class="rounded-xl border border-violet-100 bg-violet-50/40 p-3 space-y-3">
                    <p class="text-xs font-bold text-violet-700"><i class="fa-solid fa-plug"></i> {{ __('app.admin_carriers_api_settings') }}</p>
                    <div>
                        <label class="block text-xs font-semibold text-slate-700 mb-1">{{ __('app.admin_carriers_api_url') }}</label>
                    ...
                    <div class="grid grid-cols-2 gap-3">
                        <div>
                            <label class="block text-xs font-semibold text-slate-700 mb-1">{{ __('app.admin_carriers_api_key') }}</label>
                            <input type="text" name="api_key" :value="editing?.api_key" class="w-full rounded-xl border-slate-200">
                        </div>
                        <div>
                            <label class="block text-xs font-semibold text-slate-700 mb-1">{{ __('app.admin_carriers_webhook_secret') }}</label>
                            <input type="text" name="webhook_secret" :value="editing?.webhook_secret" class="w-full rounded-xl border-slate-200" placeholder="{{ __('app.admin_carriers_webhook_secret_ph') }}">
                        </div>
                    </div>
                    <label class="flex items-center gap-2">
                        <input type="checkbox" name="auto_track" value="1" :checked="editing?.auto_track" class="rounded">
                        <span class="text-xs font-semibold text-slate-700">{{ __('app.admin_carriers_auto_track') }}</span>
                    </label>
                ...
                <div class="grid grid-cols-2 gap-3">
                    <div>
                        <label class="block text-xs font-semibold text-slate-700 mb-1">{{ __('app.admin_carriers_phone') }}</label>
                        <input type="text" name="contact_phone" :value="editing?.contact_phone" class="w-full rounded-xl border-slate-200">
                    </div>
                    <div>
                        <label class="block text-xs font-semibold text-slate-700 mb-1">{{ __('app.admin_carriers_email') }}</label>
                        <input type="email" name="contact_email" :value="editing?.contact_email" class="w-full rounded-xl border-slate-200">
                    </div>
                </div>
                <div class="grid grid-cols-2 gap-3">
                    <div>
                        <label class="block text-xs font-semibold text-slate-700 mb-1">{{ __('app.admin_carriers_default_cost') }}</label>
                        <input type="number" step="0.01" name="default_cost" :value="editing?.default_cost ?? 0" class="w-full rounded-xl border-slate-200">
                    </div>
                    <div>
                        <label class="block text-xs font-semibold text-slate-700 mb-1">{{ __('app.admin_common_display_order') }}</label>
                ...
                <label class="flex items-center gap-2">
                    ...
                    <span class="text-sm font-semibold text-slate-700">{{ __('app.admin_common_active') }}</span>
                </label>
                <div class="flex gap-2 pt-4 border-t">
                    ...
                    <button type="button" @click="showForm = false" class="px-4 py-2 rounded-xl bg-slate-100 text-slate-700 font-semibold hover:bg-slate-200">{{ __('app.admin_common_cancel') }}</button>
                </div>
            </form>
        </div>
    </div>
            ...
            <tbody class="divide-y divide-slate-100">
                @forelse($carriers as $c)
                    <tr>
                        ...
                        <td class="px-4 py-3 text-slate-600">{{ money($c->default_cost) }}</td>
                        <td class="px-4 py-3">
                        ...
                        <td class="px-4 py-3">
                            <div class="flex gap-2">
                                <button @click="editing = @js($c); showForm = true" class="px-3 py-1 rounded-lg bg-sky-100 text-sky-700 text-xs font-semibold hover:bg-sky-200">{{ __('app.admin_common_edit') }}</button>
                                <form method="POST" action="{{ route('admin.shipping-carriers.destroy', $c) }}" onsubmit="return confirm('{{ __('app.admin_common_confirm_delete') }}')">
                                    @csrf @method('DELETE')
                                    <button class="px-3 py-1 rounded-lg bg-rose-100 text-rose-700 text-xs font-semibold hover:bg-rose-200">{{ __('app.admin_common_delete') }}</button>
                                </form>
                            </div>
                        </td>
                    </tr>
                @empty
                    <tr><td colspan="7" class="text-center py-12 text-slate-400">{{ __('app.admin_carriers_empty') }}</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
