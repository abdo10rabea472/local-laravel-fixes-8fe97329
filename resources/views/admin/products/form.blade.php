...
>
    ...
    <x-admin.page
        :title="$product->exists ? 'تعديل منتج معملي' : 'إضافة منتج معملي جديد'"
        subtitle="تعبئة البيانات التقنية والأسعار والـ SEO الخاصة بالأدوات والكتالوجات الجامعية."
        :back="route('admin.products.index')"
    >
        {{-- ============ MAIN COLUMN ============ --}}

        {{-- Basic Info --}}
        <x-admin.card title="المعلومات الأساسية للمنتج" icon="fa-circle-info">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    <label class="text-xs font-bold text-gray-500 dark:text-gray-400 block mb-1.5">اسم المنتج باللغة العربية *</label>
                    <input type="text" name="name" value="{{ old('name', $product->name) }}" required
                           placeholder="مثال: ميكروسكوب ضوئي ثنائي العدسات"
                ...
                <div>
                    <label class="text-xs font-bold text-gray-500 dark:text-gray-400 block mb-1.5">اسم المنتج باللغة الإنجليزية *</label>
                    <input type="text" name="name_en" value="{{ old('name_en', $product->name_en ?? '') }}"
                           placeholder="e.g., Binocular Biological Microscope"
                           class="w-full h-11 px-4 bg-gray-50 dark:bg-dark-800 border border-gray-200 dark:border-gray-700 rounded-xl text-sm focus:border-primary-500 focus:outline-none">
                </div>
                <div>
                    <label class="text-xs font-bold text-gray-500 dark:text-gray-400 block mb-1.5">الرقم التعريفي الفريد المخزني (SKU)</label>
                ...
                <div>
                    <label class="text-xs font-bold text-gray-500 dark:text-gray-400 block mb-1.5">الرابط اللطيف (Slug URL)</label>
            ...
            <div class="mt-4">
                <label class="text-xs font-bold text-gray-500 dark:text-gray-400 block mb-1.5">الوصف التفصيلي الشامل للمنتج</label>
                <textarea name="description" rows="5"
                          placeholder="اكتب المواصفات والمكونات الفنية للمجموعة المخبرية هنا..."
        ...
        {{-- Pricing & Stock --}}
        <x-admin.card title="التسعير والمخزون المالي" icon="fa-coins">
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div>
                    <label class="text-xs font-bold text-gray-500 dark:text-gray-400 block mb-1.5">سعر التكلفة الأصلي</label>
                    <input type="number" step="0.01" name="cost_price" value="{{ old('cost_price', $product->cost_price ?? '') }}"
                           placeholder="0.00 ج.م"
                           class="w-full h-11 px-4 bg-gray-50 dark:bg-dark-800 border border-gray-200 dark:border-gray-700 rounded-xl text-sm focus:border-primary-500 focus:outline-none">
                </div>
                <div>
                    <label class="text-xs font-bold text-gray-500 dark:text-gray-400 block mb-1.5">سعر البيع الأساسي *</label>
                    <input type="number" step="0.01" name="price" value="{{ old('price', $product->price) }}" required
                           placeholder="0.00 ج.م"
                           class="w-full h-11 px-4 bg-gray-50 dark:bg-dark-800 border border-gray-200 dark:border-gray-700 rounded-xl text-sm focus:border-primary-500 focus:outline-none">
                </div>
                <div>
                    <label class="text-xs font-bold text-gray-500 dark:text-gray-400 block mb-1.5">سعر الخصم للطلاب</label>
                    <input type="number" step="0.01" name="sale_price" value="{{ old('sale_price', $product->sale_price) }}"
                           placeholder="0.00 ج.م"
                           class="w-full h-11 px-4 bg-gray-50 dark:bg-dark-800 border border-gray-200 dark:border-gray-700 rounded-xl text-sm focus:border-primary-500 focus:outline-none">
                </div>
                <div>
                    <label class="text-xs font-bold text-gray-500 dark:text-gray-400 block mb-1.5">الكمية الإجمالية بالمستودع</label>
                    <input type="number" name="stock" value="{{ old('stock', $product->stock ?? 0) }}" required
                           placeholder="100"
                           class="w-full h-11 px-4 bg-gray-50 dark:bg-dark-800 border border-gray-200 dark:border-gray-700 rounded-xl text-sm focus:border-primary-500 focus:outline-none">
                </div>
                <div class="md:col-span-2">
                    <label class="text-xs font-bold text-gray-500 dark:text-gray-400 block mb-1.5">تنبيه عند انخفاض المخزون لـ</label>
        ...
        {{-- Images --}}
        <x-admin.card title="صور المنتج عالية الجودة" icon="fa-images">
            ...
            <label for="product-images" class="border-2 border-dashed border-gray-300 dark:border-gray-700 rounded-2xl p-10 text-center block cursor-pointer hover:border-primary-500 transition-colors">
                ...
                <p class="font-bold text-gray-700 dark:text-gray-200">اسحب صور المنتج وأفلتها هنا مباشرة</p>
                <p class="text-xs text-gray-400 mt-1">يدعم امتدادات PNG, JPG بحجم تصل إلى 8M (متبقي {{ $remaining }})</p>
        ...
        {{-- ============ SIDE COLUMN ============ --}}
        <x-slot:side>
            <x-admin.card title="التصنيف والانتساب للمتجر" icon="fa-sitemap">
                <div class="space-y-4">
                    <div>
                        <label class="text-xs font-bold text-gray-500 dark:text-gray-400 block mb-1.5">الكلية المعنية الأساسية</label>
                        <select class="w-full h-11 px-4 bg-gray-50 dark:bg-dark-800 border border-gray-200 dark:border-gray-700 rounded-xl text-sm focus:border-primary-500 focus:outline-none" disabled>
                            <option>كلية الطب البشري والأسنان</option>
                        </select>
                    </div>
                    <div>
                        <label class="text-xs font-bold text-gray-500 dark:text-gray-400 block mb-1.5">التصنيف الفرعي *</label>
                    ...
                    <div>
                        <label class="text-xs font-bold text-gray-500 dark:text-gray-400 block mb-1.5">العلامة التجارية</label>
                ...
                <div class="mt-5 pt-5 border-t border-gray-100 dark:border-gray-800 space-y-2">
                    <button type="submit" class="w-full h-12 bg-primary-600 hover:bg-primary-700 text-white font-bold rounded-xl shadow-lg shadow-primary-500/20 transition-colors">
                        <i class="fas fa-rocket ml-1"></i> {{ $product->exists ? 'تحديث المنتج' : 'نشر وتثبيت المنتج بالموقع' }}
                    </button>
