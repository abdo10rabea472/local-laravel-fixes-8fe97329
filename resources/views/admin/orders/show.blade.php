...
<div class="p-6 max-w-6xl mx-auto" x-data="orderShow({{ $order->id }}, '{{ $order->status }}')">
    <div class="flex items-center justify-between mb-6 flex-wrap gap-3">
        <div>
            <h1 class="text-2xl font-black text-slate-900">Order <span class="font-mono text-violet-700">{{ $order->order_number }}</span></h1>
            <p class="text-sm text-slate-500 mt-1">{{ $order->created_at->format('Y-m-d H:i') }}</p>
        </div>
        <div class="flex gap-2">
            <a href="{{ route('admin.orders.invoice', $order) }}" target="_blank" class="px-4 py-2 bg-slate-900 text-white rounded-xl text-sm font-bold"><i class="fa-solid fa-print mr-1"></i> Print invoice</a>
            <button @click="resendEmail()" :disabled="busy" class="px-4 py-2 bg-emerald-600 hover:bg-emerald-700 disabled:opacity-50 text-white rounded-xl text-sm font-bold"><i class="fa-solid fa-envelope mr-1"></i> Email customer</button>
            <a href="{{ route('admin.orders.index') }}" class="px-4 py-2 bg-slate-100 text-slate-700 rounded-xl text-sm font-bold">Back</a>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div class="lg:col-span-2 space-y-6">
            ...
            <div class="bg-white rounded-2xl border border-slate-200 overflow-hidden">
                <div class="p-4 border-b border-slate-100 font-bold">Items</div>
                <table class="w-full text-sm">
                    <thead class="bg-slate-50 text-xs text-slate-600"><tr>
                        <th class="p-3 text-left">Product</th><th class="p-3">Price</th><th class="p-3">Qty</th><th class="p-3">Total</th>
                    </tr></thead>
                    <tbody>
                    ...
                    <tfoot class="bg-slate-50 text-sm">
                        <tr><td colspan="3" class="p-2 text-right">Subtotal</td><td class="p-2 text-center">{{ number_format($order->subtotal, 2) }}</td></tr>
                        @if($order->discount_amount > 0)
                        <tr><td colspan="3" class="p-2 text-right">Discount {{ $order->coupon_code ? '(' . $order->coupon_code . ')' : '' }}</td><td class="p-2 text-center text-rose-600">-{{ number_format($order->discount_amount, 2) }}</td></tr>
                        @endif
                        <tr><td colspan="3" class="p-2 text-right">Shipping</td><td class="p-2 text-center">{{ number_format($order->shipping_cost, 2) }}</td></tr>
                        <tr><td colspan="3" class="p-2 text-right font-bold">Final Total</td><td class="p-2 text-center font-black text-violet-700 text-lg">{{ number_format($order->total, 2) }} {{ $order->currency }}</td></tr>
                    </tfoot>
                </table>
            </div>
            ...
            <div class="bg-white rounded-2xl border border-slate-200 p-4">
                <h3 class="font-bold mb-3">Status History</h3>
        ...
        <div class="space-y-6">
            ...
            <div class="bg-white rounded-2xl border border-slate-200 p-4 text-sm">
                ...
                @endif
                @if($order->notes)
                <hr class="my-3"><p class="text-slate-600"><b>Notes:</b> {{ $order->notes }}</p>
                @endif
            </div>
            ...
            <div class="bg-white rounded-2xl border border-slate-200 p-4">
                ...
                <input type="text" x-model="note" placeholder="Note (optional)" class="w-full h-10 px-3 border border-slate-200 rounded-xl text-sm mb-2">
                <label class="flex items-center gap-2 text-xs text-slate-600 mb-3">
                    <input type="checkbox" x-model="notify"> Auto-email customer
                </label>
            ...
            <div class="bg-white rounded-2xl border border-slate-200 p-4">
                <h3 class="font-bold mb-3 flex items-center gap-2">
                    <i class="fa-solid fa-truck-fast text-violet-600"></i> Shipment
                ...
                @if($order->shipping_error)
                    <div class="mb-3 p-3 rounded-xl bg-rose-50 border border-rose-200 text-rose-700 text-xs">
                        <p class="font-bold mb-1"><i class="fa-solid fa-triangle-exclamation"></i> Failed to create shipment</p>
                        <p class="break-words">{{ $order->shipping_error }}</p>
                        <p class="mt-1 text-[10px] opacity-70">Attempts: {{ $order->shipping_attempts }}</p>
                    </div>
                @endif

                <div class="text-xs space-y-1 mb-3">
                    <p><b>Carrier:</b> {{ $order->carrier->name }}</p>
                    @if($order->shipment_number)
                        <p><b>Shipment #:</b> <span class="font-mono">{{ $order->shipment_number }}</span></p>
                    @endif
                    @if($order->tracking_number)
                        <p><b>Tracking #:</b> <span class="font-mono">{{ $order->tracking_number }}</span></p>
                    @endif
                    @if($order->shipment_created_at)
                        <p><b>Created at:</b> {{ $order->shipment_created_at->format('Y-m-d H:i') }}</p>
                    @endif
                </div>

                @if($order->barcode)
                    <div class="mb-3 p-2 bg-white rounded-lg border border-slate-200 text-center">
                        <p class="text-[10px] text-slate-500 mb-1">Barcode</p>
                ...
                @if($order->pickup_address)
                    <div class="mb-3 p-2 rounded-lg bg-slate-50 text-xs">
                        <p class="font-bold mb-1">Pickup Address</p>
                        ...
                        @if($order->pickup_datetime)
                            <p class="mt-1"><b>Pickup time:</b> {{ $order->pickup_datetime->format('Y-m-d H:i') }}</p>
                        @endif
                    </div>
                @endif
                    ...
                    @if($order->label_url)
                        <a href="{{ $order->label_url }}" target="_blank" rel="noopener"
                           class="h-9 inline-flex items-center justify-center bg-violet-600 hover:bg-violet-700 text-white rounded-xl font-bold text-xs">
                            <i class="fa-solid fa-print mr-1"></i> Print label
                        </a>
                    ...
                    @if($order->tracking_url)
                        <a href="{{ $order->tracking_url }}" target="_blank" rel="noopener"
                           class="col-span-2 h-9 inline-flex items-center justify-center bg-emerald-50 hover:bg-emerald-100 text-emerald-700 rounded-xl font-bold text-xs">
                            <i class="fa-solid fa-location-arrow mr-1"></i> Carrier tracking page
                        </a>
                    @endif
                    @if($order->shipment_number)
                        <form method="POST" action="{{ route('admin.orders.shipment.sync', $order) }}" class="col-span-2">@csrf
                            <button type="submit" class="w-full h-9 bg-sky-50 hover:bg-sky-100 text-sky-700 rounded-xl font-bold text-xs">
                                <i class="fa-solid fa-arrows-rotate mr-1"></i> Sync shipment status
                            </button>
                        </form>
                    @endif
                    @if(!$order->shipment_number && $order->shipping_carrier_id)
                        <form method="POST" action="{{ route('admin.orders.shipment.retry', $order) }}" class="col-span-2">@csrf
                            <button type="submit" class="w-full h-9 bg-amber-500 hover:bg-amber-600 text-white rounded-xl font-bold text-xs">
                                <i class="fa-solid fa-rotate mr-1"></i> Retry creating shipment
                            </button>
                        </form>
                    @endif
                </div>
                ...
                @if(!empty($order->tracking_history) && is_array($order->tracking_history))
                    <div class="mt-3 border-t pt-3 max-h-40 overflow-y-auto">
                        <p class="text-[10px] font-bold text-slate-500 mb-2">Tracking history</p>
                        @foreach(array_reverse($order->tracking_history) as $ev)
                            <div class="text-[11px] text-slate-600 border-l-2 border-violet-200 pl-2 py-1 mb-1">
            ...
            <div class="bg-white rounded-2xl border border-slate-200 p-4">
                <h3 class="font-bold mb-3">Manual Shipping Override</h3>
                <label class="block text-xs text-slate-500 mb-1">Carrier</label>
                ...
                <input type="text" x-model="tracking" placeholder="Tracking number" class="w-full h-10 px-3 border border-slate-200 rounded-xl text-sm mb-2">
                <label class="block text-xs text-slate-500 mb-1">Actual cost</label>
                ...
                <label class="block text-xs text-slate-500 mb-1">Shipping notes</label>
                <textarea x-model="shipNotes" rows="2" class="w-full px-3 py-2 border border-slate-200 rounded-xl text-sm mb-2"></textarea>
                <button @click="updateShipping()" :disabled="busy" class="w-full h-10 bg-slate-900 hover:bg-slate-800 disabled:opacity-50 text-white rounded-xl font-bold text-sm">Save shipping data</button>
            </div>
    ...
    return {
        ...
        async req(url, method, body){
            ...
            try{
                ...
                if(j.ok){ this.toast(j.message || 'Done', true); setTimeout(()=>location.reload(), 600); }
                else this.toast(j.message || 'An error occurred', false);
            } catch(e){ this.toast('Connection error', false); }
