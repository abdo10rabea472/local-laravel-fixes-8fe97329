...
@section('title', __('app.admin_subcats_title'))

@section('content')
<x-admin.page :title="__('app.admin_subcats_heading')" :subtitle="__('app.admin_subcats_subtitle')">
    <x-admin.card :title="__('app.admin_subcats_card_title')" icon="fa-sitemap" padding="p-0">
        <div class="overflow-x-auto">
            <table class="w-full text-sm">
                <thead class="bg-gray-50 dark:bg-dark-800 text-gray-500 dark:text-gray-400 text-xs">
                    <tr>
                        <th class="text-left p-4">{{ __('app.admin_subcats_col_category') }}</th>
                        <th class="text-left p-4">{{ __('app.admin_subcats_col_college') }}</th>
                    ...
                    <tr class="border-t border-gray-100 dark:border-gray-800 hover:bg-gray-50 dark:hover:bg-dark-800/50">
                        ...
                        <td class="p-4">
                            <span class="px-2 py-1 rounded-full text-xs font-bold {{ $sub->status ? 'bg-emerald-50 dark:bg-emerald-950/30 text-emerald-700 dark:text-emerald-400' : 'bg-gray-100 dark:bg-dark-800 text-gray-500' }}">
                                {{ $sub->status ? __('app.admin_common_active') : __('app.admin_common_disabled') }}
                            </span>
                        </td>
                        <td class="p-4">
                            <div class="flex gap-3">
                                ...
                                <form method="POST" action="{{ route('admin.subcategories.destroy', $sub) }}" data-ajax-confirm="{{ __('app.admin_subcats_confirm_delete') }}" data-ajax-remove>
                                    @csrf @method('DELETE')
    ...
    <x-slot:side>
        <x-admin.card :title="__('app.admin_common_quick_actions')" icon="fa-bolt">
            <a href="{{ route('admin.subcategories.create') }}" class="w-full h-12 inline-flex items-center justify-center gap-2 bg-primary-600 hover:bg-primary-700 text-white font-bold rounded-xl shadow-lg shadow-primary-500/20 transition-colors">
                <i class="fa-solid fa-plus"></i> {{ __('app.admin_subcats_add') }}
            </a>
        </x-admin.card>

        <x-admin.card :title="__('app.admin_common_filter')" icon="fa-filter">
            <form method="GET" class="space-y-3">
                <div>
                    ...
                    <select name="college_id" class="w-full h-11 px-4 bg-gray-50 dark:bg-dark-800 border border-gray-200 dark:border-gray-700 rounded-xl text-sm focus:border-primary-500 focus:outline-none">
                        <option value="">{{ __('app.admin_subcats_all_colleges') }}</option>
