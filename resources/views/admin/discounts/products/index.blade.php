...
<div class="p-6 max-w-7xl mx-auto" x-data="{ openForm: false, edit: null }">
    ...
    <div class="flex items-center justify-between mb-6">
        <div>
            <h1 class="text-2xl font-black text-slate-900">خصومات المنتجات</h1>
            <p class="text-sm text-slate-500 mt-1">إدارة الخصومات المباشرة على المنتجات (نسبة أو قيمة ثابتة).</p>
        </div>
        <button @click="openForm = true; edit = null" class="px-5 py-2.5 bg-violet-600 hover:bg-violet-700 text-white font-bold rounded-xl">
            <i class="fa-solid fa-plus ml-1"></i> إضافة خصم
        </button>
    </div>
    ...
    <form method="GET" class="flex gap-2 mb-4">
        <select name="status" class="h-10 px-3 border border-slate-200 rounded-xl text-sm">
            ...
            <option value="active" @selected(request('status')==='active')>مفعل وساري</option>
        ...
        <table class="w-full text-sm">
            ...
            <tbody>
                ...
                <tr class="border-t border-slate-100">
                    ...
                    <td class="p-3 text-center">{{ $d->type === 'percent' ? 'نسبة %' : 'قيمة ثابتة' }}</td>
                    ...
                    <td class="p-3 text-center">
                        @if($d->isCurrentlyActive())
                            <span class="px-2 py-1 text-xs bg-emerald-50 text-emerald-700 rounded-full font-bold">نشط</span>
                        @elseif(! $d->is_active)
                            <span class="px-2 py-1 text-xs bg-slate-100 text-slate-600 rounded-full font-bold">معطل</span>
                        @else
                            <span class="px-2 py-1 text-xs bg-amber-50 text-amber-700 rounded-full font-bold">خارج المدة</span>
                        @endif
                    </td>
                    <td class="p-3 text-center">
                ...
                @empty
                <tr><td colspan="7" class="p-8 text-center text-slate-400">لا توجد خصومات بعد.</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
    ...
    <div x-show="openForm" x-cloak class="fixed inset-0 bg-slate-900/60 z-50 flex items-center justify-center p-4" @click.self="openForm = false">
        <div class="bg-white rounded-3xl max-w-lg w-full p-6">
            ...
            <form :action="edit ? `{{ url('admin/product-discounts') }}/${edit.id}` : `{{ route('admin.product-discounts.store') }}`" method="POST" class="space-y-3" data-ajax data-success-toast="تم الحفظ بنجاح" @ul:success.window="openForm = false; setTimeout(()=>window.location.reload(), 400)">
                @csrf
                <template x-if="edit">@method('PUT')</template>
                <input type="hidden" name="_method" :value="edit ? 'PUT' : 'POST'">

                <div>
                    <label class="text-xs font-bold text-slate-500 block mb-1">المنتج</label>
                ...
                <div class="grid grid-cols-2 gap-3">
                    <div>
                        <label class="text-xs font-bold text-slate-500 block mb-1">النوع</label>
                        <select name="type" class="w-full h-11 px-3 border border-slate-200 rounded-xl text-sm">
                            <option value="percent" x-bind:selected="!edit || edit.type === 'percent'">نسبة %</option>
                            <option value="fixed" x-bind:selected="edit && edit.type === 'fixed'">قيمة ثابتة</option>
                        </select>
                    </div>
                    <div>
                ...
                <div class="grid grid-cols-2 gap-3">
                    <div>
                        <label class="text-xs font-bold text-slate-500 block mb-1">يبدأ في</label>
                    ...
                    <div>
                        <label class="text-xs font-bold text-slate-500 block mb-1">ينتهي في</label>
                ...
                <label class="flex items-center gap-2 text-sm">
                    <input type="checkbox" name="is_active" value="1" x-bind:checked="!edit || edit.is_active"> مفعل
                </label>
                <div class="flex gap-2 pt-2">
                    <button class="px-5 py-2.5 bg-violet-600 text-white font-bold rounded-xl">حفظ</button>
                    <button type="button" @click="openForm = false" class="px-5 py-2.5 bg-slate-100 text-slate-700 font-bold rounded-xl">إلغاء</button>
                </div>
            </form>
        </div>
    </div>
