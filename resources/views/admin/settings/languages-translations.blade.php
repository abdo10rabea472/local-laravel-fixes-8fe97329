...
<div class="space-y-6"
 ...
 @if(session('success'))<div class="p-4 rounded-2xl bg-emerald-50 dark:bg-emerald-950/30 text-emerald-700 dark:text-emerald-300 text-sm font-bold">{{ session('success') }}</div>@endif
 @if(session('error'))<div class="p-4 rounded-2xl bg-rose-50 dark:bg-rose-950/30 text-rose-700 dark:text-rose-300 text-sm font-bold">{{ session('error') }}</div>@endif

 <div class="bg-white dark:bg-dark-900 border border-slate-200 dark:border-gray-700 rounded-3xl overflow-hidden">
 {{-- Header --}}
 <div class="flex items-center justify-between p-5 border-b border-slate-100 dark:border-gray-800 flex-wrap gap-3">
 <div>
 <h2 class="text-lg font-black text-slate-800 dark:text-gray-100">
 <i class="fa-solid fa-language mr-2 text-violet-600"></i>
 {{ $language->name }} <span class="text-slate-400 dark:text-gray-500">({{ $language->code }})</span>
 </h2>
 ...
 <a href="{{ route('admin.settings.languages.index') }}" class="text-sm text-slate-500 dark:text-gray-400 hover:text-slate-800 dark:hover:text-gray-100">
 <i class="fa-solid fa-arrow-left mr-1"></i> {{ __('app.admin_settings_translations_back') }}
 </a>
 </div>

 @if(empty($groupNames))
 <div class="p-8 text-center text-slate-400 dark:text-gray-500">
 ...
 {{-- Groups sidebar --}}
 <aside class="col-span-12 md:col-span-3 lg:col-span-2 border-e border-slate-100 dark:border-gray-800 bg-slate-50 dark:bg-dark-800 max-h-[80vh] overflow-y-auto">
 <div class="p-3 border-b border-slate-100 dark:border-gray-800 sticky top-0 bg-slate-50 dark:bg-dark-800 z-10">
 <input type="text" id="group-filter" placeholder="Filter groups…"
 class="w-full h-9 px-3 bg-white dark:bg-dark-900 border border-slate-200 dark:border-gray-700 rounded-lg text-xs">
 </div>
 ...
 {{ $selected === $g ? 'bg-violet-600 text-white' : 'text-slate-700 dark:text-gray-200 hover:bg-white dark:hover:bg-dark-900' }}">
 ...
 <div class="p-2 border-t border-slate-100 dark:border-gray-800">
 ...
 @csrf
 <div class="p-5 space-y-3 bg-emerald-50/40 dark:bg-emerald-950/20 min-h-[400px]" x-data="newKeysPanel('{{ $groupNames[0] ?? '' }}')">
 <div class="flex items-center justify-between">
 <div>
 <h3 class="text-sm font-black text-slate-800 dark:text-gray-100">{{ __('app.admin_settings_translations_add_keys_title') }}</h3>
 <p class="text-xs text-slate-500 dark:text-gray-400">{{ __('app.admin_settings_translations_add_keys_subtitle') }}</p>
 </div>
 ...
 <div class="grid grid-cols-12 gap-2 text-[11px] font-bold text-slate-500 dark:text-gray-400 uppercase px-1">
 ...
 class="col-span-2 px-3 py-2 bg-white dark:bg-dark-900 border border-slate-200 dark:border-gray-700 rounded-xl text-sm font-mono">
 <input type="text" :name="`new[${i}][key]`" x-model="row.key" placeholder="my_new_key"
 class="col-span-3 px-3 py-2 bg-white dark:bg-dark-900 border border-slate-200 dark:border-gray-700 rounded-xl text-sm font-mono">
 <textarea :name="`new[${i}][en]`" x-model="row.en" rows="1" dir="ltr" placeholder="English text…"
 class="col-span-3 px-3 py-2 bg-white dark:bg-dark-900 border border-slate-200 dark:border-gray-700 rounded-xl text-sm"></textarea>
 <textarea :name="`new[${i}][value]`" x-model="row.value" rows="1" dir="auto"
 :placeholder="`Translation in {{ $language->native_name }}…`"
 class="col-span-3 px-3 py-2 bg-white dark:bg-dark-900 border border-slate-200 dark:border-gray-700 rounded-xl text-sm"></textarea>
 <button type="button" @click="removeRow(i)"
 class="col-span-1 h-10 rounded-xl bg-rose-50 dark:bg-rose-950/30 text-rose-600 dark:text-rose-400 hover:bg-rose-100 dark:hover:bg-rose-900/40 text-sm">
 ...
 <div class="p-4 border-t border-slate-100 dark:border-gray-800 flex justify-end gap-2 bg-slate-50 dark:bg-dark-800">
 <a href="{{ route('admin.settings.languages.index') }}" class="px-4 py-2 rounded-xl bg-white dark:bg-dark-900 border border-slate-200 dark:border-gray-700 text-sm font-bold text-slate-700 dark:text-gray-200">{{ __('app.admin_settings_translations_btn_cancel') }}</a>
 ...
 @else
 <form method="GET" class="flex flex-wrap items-center gap-2 p-3 border-b border-slate-100 dark:border-gray-800 bg-slate-50 dark:bg-dark-800">
 ...
 class="flex-1 min-w-[200px] h-9 px-3 bg-white dark:bg-dark-900 border border-slate-200 dark:border-gray-700 rounded-xl text-sm">
 <select name="per_page" class="h-9 px-2 bg-white dark:bg-dark-900 border border-slate-200 dark:border-gray-700 rounded-xl text-sm">
 ...
 <span class="text-xs text-slate-500 dark:text-gray-400 ms-auto">
 <span class="font-bold text-slate-700 dark:text-gray-200">{{ $rendered['untranslated'] }}</span> untranslated of
 <span class="font-bold">{{ $counts[$selected] ?? 0 }}</span>
 </span>
 </form>

 <div class="flex flex-wrap gap-2 px-4 py-2 border-b border-slate-100 dark:border-gray-800 items-center">
 ...
 class="h-8 px-3 rounded-lg bg-white dark:bg-dark-900 border border-violet-300 dark:border-violet-700 text-violet-700 dark:text-violet-300 text-xs font-bold">
 ...
 <span class="text-xs text-violet-700 dark:text-violet-300"><i class="fa-solid fa-spinner fa-spin mr-1"></i>
 ...
 <span class="text-xs text-emerald-700 dark:text-emerald-300"><i class="fa-solid fa-check mr-1"></i><span x-text="lastMessage"></span></span>
 </template>
 <template x-if="lastError">
 <span class="text-xs text-rose-700 dark:text-rose-300 ms-auto"><i class="fa-solid fa-triangle-exclamation mr-1"></i><span x-text="lastError"></span></span>
 </template>
 </div>
 ...
 @endphp
 <div class="p-4 grid grid-cols-12 gap-4 items-start hover:bg-slate-50/50 dark:hover:bg-dark-800/50">
 <div class="col-span-12 md:col-span-5">
 <div class="flex items-center gap-2">
 <code class="text-[11px] text-slate-500 dark:text-gray-400 font-mono break-all">{{ $selected }}.{{ $key }}</code>
 @if($empty)
 <span class="text-[10px] font-bold px-1.5 py-0.5 rounded bg-amber-100 dark:bg-amber-900/40 text-amber-700 dark:text-amber-300">empty</span>
 @endif
 </div>
 <div class="text-sm text-slate-700 dark:text-gray-200 mt-1.5 leading-snug" dir="auto">{{ $source }}</div>
 </div>
 <div class="col-span-12 md:col-span-7">
 <textarea name="t[{{ $selected }}][{{ $key }}]" rows="2" dir="auto"
 data-ai-group="{{ $selected }}" data-ai-key="{{ $key }}"
 data-ai-source="{{ e($source) }}"
 class="ai-target w-full px-3 py-2 bg-slate-50 dark:bg-dark-800 border {{ $empty ? 'border-amber-300 dark:border-amber-700' : 'border-slate-200 dark:border-gray-700' }} rounded-xl text-sm focus:bg-white dark:focus:bg-dark-900 focus:border-violet-400 dark:focus:border-violet-500"
 placeholder="Translation in {{ $language->native_name }}…">{{ $value }}</textarea>
 </div>
 </div>
 @empty
 <div class="p-12 text-center text-slate-400 dark:text-gray-500 text-sm">No keys match this search.</div>
 @endforelse
 </div>

 {{-- Pagination + save --}}
 <div class="p-4 border-t border-slate-100 dark:border-gray-800 flex flex-wrap items-center justify-between gap-3 bg-slate-50 dark:bg-dark-800">
 ...
 class="px-3 h-9 inline-flex items-center bg-white dark:bg-dark-900 border border-slate-200 dark:border-gray-700 rounded-lg text-xs font-bold {{ $page<=1 ? 'opacity-40 pointer-events-none' : '' }}">‹</a>
 @if($start > 1)
 <a href="{{ route('admin.settings.languages.translations', ['language' => $language->id] + $baseQs + ['page' => 1]) }}"
 class="px-3 h-9 inline-flex items-center bg-white dark:bg-dark-900 border border-slate-200 dark:border-gray-700 rounded-lg text-xs font-bold">1</a>
 @if($start > 2)<span class="px-1 text-slate-400 dark:text-gray-500">…</span>@endif
 @endif
 @for($p = $start; $p <= $end; $p++)
 <a href="{{ route('admin.settings.languages.translations', ['language' => $language->id] + $baseQs + ['page' => $p]) }}"
 class="px-3 h-9 inline-flex items-center rounded-lg text-xs font-bold {{ $p===$page ? 'bg-violet-600 text-white' : 'bg-white dark:bg-dark-900 border border-slate-200 dark:border-gray-700' }}">{{ $p }}</a>
 @endfor
 @if($end < $rendered['lastPage'])
 @if($end < $rendered['lastPage']-1)<span class="px-1 text-slate-400 dark:text-gray-500">…</span>@endif
 <a href="{{ route('admin.settings.languages.translations', ['language' => $language->id] + $baseQs + ['page' => $rendered['lastPage']]) }}"
 class="px-3 h-9 inline-flex items-center bg-white dark:bg-dark-900 border border-slate-200 dark:border-gray-700 rounded-lg text-xs font-bold">{{ $rendered['lastPage'] }}</a>
 @endif
 <a href="{{ route('admin.settings.languages.translations', ['language' => $language->id] + $baseQs + ['page' => min($rendered['lastPage'],$page+1)]) }}"
 class="px-3 h-9 inline-flex items-center bg-white dark:bg-dark-900 border border-slate-200 dark:border-gray-700 rounded-lg text-xs font-bold {{ $page>=$rendered['lastPage'] ? 'opacity-40 pointer-events-none' : '' }}">›</a>
 @endif
 </div>
...
document.addEventListener('alpine:init', () => {
 ...
 ta.classList.add('!bg-emerald-50 dark:bg-emerald-950/30');
 setTimeout(() => ta.classList.remove('!bg-emerald-50 dark:bg-emerald-950/30'), 600);
 this.progress.done++;
 } catch (e) {
