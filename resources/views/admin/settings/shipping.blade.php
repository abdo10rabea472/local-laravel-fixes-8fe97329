...
<div class="space-y-6"
    ...
    <div class="bg-white border border-slate-200 rounded-3xl shadow-sm overflow-hidden">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 px-6 py-4 border-b border-slate-100">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <h3 class="text-base font-bold text-slate-800">الدول والمناطق</h3>
                    <p class="text-xs text-slate-500 mt-0.5">إدارة الدول والمحافظات / المدن / الولايات وأسعار الشحن.</p>
                </div>
            </div>
            <button @click="openCreateCountry()" class="bg-violet-600 hover:bg-violet-700 text-white font-bold text-sm px-5 py-2.5 rounded-xl shadow-md shadow-violet-500/20 transition-colors">
                <i class="fa-solid fa-plus ml-2"></i> إضافة دولة
            </button>
        </div>
        ...
        <div class="divide-y divide-slate-100">
                ...
                <div class="flex flex-wrap items-center justify-between gap-3">
                    <div class="flex items-center gap-3 flex-1 min-w-0">
                        ...
                        <div class="min-w-0">
                            <div class="flex items-center gap-2 flex-wrap">
                                ...
                                @if($country->status)
                                    <span class="text-[10px] font-bold text-emerald-700 bg-emerald-50 px-2 py-0.5 rounded-full border border-emerald-200">نشط</span>
                                @else
                                    <span class="text-[10px] font-bold text-slate-600 bg-slate-100 px-2 py-0.5 rounded-full">معطل</span>
                                @endif
                                <span class="text-[10px] font-bold text-slate-500 bg-slate-100 px-2 py-0.5 rounded-full">
                                    {{ $country->regions->count() }} منطقة
                                </span>
                            </div>
                            <p class="text-xs text-slate-500 mt-1">
                                @if($country->cost !== null)
                                    سعر الدولة: <span class="font-bold text-violet-600">{{ number_format($country->cost, 2) }} EGP</span>
                                @else
                                    <span class="italic">بدون سعر دولة (يحتسب سعر المنطقة فقط)</span>
                                @endif
                            </p>
                        </div>
                    </div>
                    <div class="flex items-center gap-2">
                        <button @click="openCreateRegion({{ $country->id }})" class="text-xs font-bold bg-emerald-50 hover:bg-emerald-100 text-emerald-700 px-3 py-2 rounded-xl">
                            <i class="fa-solid fa-plus ml-1"></i> منطقة
                        </button>
                ...
                <div x-show="open" x-collapse class="mt-4 pl-12">
                    @if($country->regions->isEmpty())
                        <div class="text-center text-sm text-slate-400 py-6 bg-slate-50/50 rounded-2xl border border-dashed border-slate-200">
                            ...
                            <p>لا توجد مناطق لهذه الدولة بعد.</p>
                        </div>
                    @else
                    <div class="overflow-x-auto rounded-2xl border border-slate-100">
                        <table class="w-full text-sm">
                            <thead class="bg-slate-50 text-slate-500 text-xs font-bold uppercase">
                                <tr>
                                    <th class="px-4 py-2.5 text-right">المنطقة (محافظة / مدينة / ولاية)</th>
                                    <th class="px-4 py-2.5 text-right">سعر المنطقة</th>
                                    <th class="px-4 py-2.5 text-right">الإجمالي مع الدولة</th>
                                    <th class="px-4 py-2.5 text-right">الحالة</th>
                                ...
                                @foreach($country->regions as $region)
                                    ...
                                    <tr>
                                        ...
                                        <td class="px-4 py-3 text-slate-700">
                                            {{ $region->cost !== null ? number_format($region->cost, 2) . ' EGP' : '—' }}
                                        </td>
                                        <td class="px-4 py-3 font-bold text-violet-600">{{ number_format($total, 2) }} EGP</td>
                                        <td class="px-4 py-3">
                                            @if($region->status)
                                                <span class="text-[10px] font-bold text-emerald-700 bg-emerald-50 px-2 py-0.5 rounded-full border border-emerald-200">نشط</span>
                                            @else
            ...
            <div class="p-12 text-center text-slate-500">
                ...
                <p class="font-semibold">لا توجد دول مضافة بعد.</p>
                <p class="text-xs text-slate-400 mt-1">ابدأ بإضافة دولة، ثم أضف مناطقها.</p>
            </div>
            @endforelse
        </div>
    </div>
    ...
    <div x-show="showCountryModal" x-cloak class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50">
        <div class="bg-white rounded-3xl shadow-2xl w-full max-w-lg p-6" @click.outside="showCountryModal = false">
            <h4 class="text-lg font-bold mb-4" x-text="isEditCountry ? 'تعديل الدولة' : 'إضافة دولة'"></h4>
            <form :action="isEditCountry ? '{{ url('admin/settings/shipping/countries') }}/' + country.id : '{{ route('admin.settings.shipping.countries.store') }}'" method="POST" class="space-y-4">
                ...
                <div class="space-y-2">
                    <label class="text-xs font-bold text-slate-500">اسم الدولة *</label>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div class="space-y-2">
                        <label class="text-xs font-bold text-slate-500">سعر الدولة (EGP) — اختياري</label>
                        ...
                        <p class="text-[11px] text-slate-400">إذا تم تحديده يُجمع مع سعر المنطقة.</p>
                    </div>
                    <div class="space-y-2">
                ...
                <div class="flex gap-3 pt-4">
                    <button type="submit" class="flex-1 h-11 bg-violet-600 hover:bg-violet-700 text-white font-bold rounded-2xl">حفظ</button>
                    <button type="button" @click="showCountryModal = false" class="h-11 px-6 bg-slate-100 rounded-2xl font-bold">إلغاء</button>
                </div>
            </form>
        </div>
    </div>
    ...
    <div x-show="showRegionModal" x-cloak class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50">
        <div class="bg-white rounded-3xl shadow-2xl w-full max-w-lg p-6" @click.outside="showRegionModal = false">
            <h4 class="text-lg font-bold mb-4" x-text="isEditRegion ? 'تعديل المنطقة' : 'إضافة منطقة'"></h4>
            <form :action="isEditRegion ? '{{ url('admin/settings/shipping/regions') }}/' + region.id : '{{ route('admin.settings.shipping.regions.store') }}'" method="POST" class="space-y-4">
                ...
                <div class="space-y-2">
                    <label class="text-xs font-bold text-slate-500">اسم المنطقة (محافظة / مدينة / ولاية) *</label>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div class="space-y-2">
                        <label class="text-xs font-bold text-slate-500">سعر المنطقة (EGP) — اختياري</label>
                        ...
                        <p class="text-[11px] text-slate-400">يُضاف إلى سعر الدولة (إن وُجد).</p>
                    </div>
                    <div class="space-y-2">
                        <label class="text-xs font-bold text-slate-500">الترتيب</label>
                ...
                <div class="flex gap-3 pt-4">
                    <button type="submit" class="flex-1 h-11 bg-emerald-500 hover:bg-emerald-600 text-white font-bold rounded-2xl">حفظ</button>
                    <button type="button" @click="showRegionModal = false" class="h-11 px-6 bg-slate-100 rounded-2xl font-bold">إلغاء</button>
                </div>
            </form>
        </div>
    </div>
