...
<div class="p-6 space-y-6">
    <h1 class="text-2xl font-bold text-slate-800 dark:text-white">{{ __('app.admin_returns_heading') }}</h1>

    {{-- Stats --}}
    <div class="grid grid-cols-2 lg:grid-cols-5 gap-4">
        <div class="bg-white rounded-2xl shadow-sm border p-5">
            <p class="text-xs text-slate-500 font-semibold">{{ __('app.admin_returns_stat_total') }}</p>
    ...
    <form method="GET" class="bg-white rounded-2xl shadow-sm border p-4 flex flex-wrap items-center gap-3">
        <input type="text" name="q" value="{{ $search }}" placeholder="{{ __('app.admin_returns_search_ph') }}" class="flex-1 min-w-[200px] rounded-xl border-slate-200 text-sm">
                ...
                <tbody class="divide-y divide-slate-100">
                    @forelse($returns as $r)
                        <tr class="hover:bg-slate-50">
                            ...
                            <td class="px-4 py-3 font-semibold text-slate-800">{{ money($r->refund_amount) }}</td>
                            <td class="px-4 py-3">
                            ...
                            <td class="px-4 py-3">
                                <a href="{{ route('admin.returns.show', $r) }}" class="px-3 py-1 rounded-lg bg-violet-100 text-violet-700 text-xs font-semibold hover:bg-violet-200">{{ __('app.admin_returns_manage') }}</a>
                            </td>
                        </tr>
                    @empty
                        <tr><td colspan="7" class="text-center py-12 text-slate-400">{{ __('app.admin_returns_empty') }}</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
